...
de referencia (MapBiomas, Hansen GFC, Google Earth, indices espectrales).
...
def get_reference_lulc(year, region):
    ...
    Se usa como base para muestreo estratificado.
    """
    # --- Hansen GFC: separar bosque denso ---
    ...
    # Cap year_offset at 22 so that the most recent period still has
    # ~2 years of recent-loss pixels qualifying as potential forest,
    # consistent with the buffer available for earlier periods.
    year_offset = min(year - 2000, 22)
    forest_dense = treecover2000.gte(60).And(
        loss.eq(0).Or(loss.gt(year_offset))
    )
    ...
    # --- GHSL: areas urbanas ---
    # GHSL-SMOD: grado de urbanizacion (codes: 10-13=rural, 21-23=suburban, 30=centre)
    ghsl = ee.Image('JRC/GHSL/P2023A/GHS_SMOD/2020').select('smod_code')
    urban = ghsl.gte(30)  # smod_code >= 30 = urban centres only

    # --- Combinar en mapa de referencia ---
    # Prioridad: agua > urbano > bosque denso > bosque secundario > resto (pasturas default)
...
def get_mapbiomas_reference(year, region):
    ...
    try:
        ...
        # Reclasificacion MapBiomas -> 7 clases
        # MapBiomas clases principales:
        # 3=Forest, 4=Savanna, 12=Grassland, 15=Pasture,
...
def generate_stratified_samples(reference_map, region, n_per_class=500, seed=42):
    ...
    Args:
        reference_map: ee.Image con valores 1-7
...
def main():
    ...
    for period_key, period_info in PERIODS.items():
        ...
        n_per_class = 500
        print(f"  Muestreo estratificado: {n_per_class} puntos/clase x 7 clases = {n_per_class * 7} total")
