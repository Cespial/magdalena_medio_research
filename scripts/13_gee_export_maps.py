...
def export_lulc_classifications(region, output_dir, scale=100):
    ...
    Uses rule-based classification from Hansen GFC + JRC + GHSL.
    This is a lightweight computation that avoids the RF pipeline complexity
    (band type conflicts, computation timeouts) while producing visually
    representative maps. Quantitative data comes from saved JSON results.
    """
    ...
    for period_key, year in periods.items():
        ...
        # Dense forest: treecover2000 >= 60 AND not lost by year
        # Also include gain pixels for later periods
        dense_forest = treecover.gte(60).And(lost_by_year.Not())
        if year >= 2016:
            dense_forest = dense_forest.Or(
                gain.eq(1).And(lost_by_year.Not()).And(treecover.gte(40))
            )

        # Secondary forest: treecover 25-60 AND not lost
        secondary_forest = treecover.gte(25).And(treecover.lt(60)) \
            .And(lost_by_year.Not()).And(dense_forest.Not())

        # Water: JRC occurrence > 80%
        water = jrc.select('occurrence').gt(80)

        # Urban: GHSL SMOD >= 20 (urban centres + dense urban)
        ...
        # Build classified image (class values: 1-Dense, 2-Secondary, 3-Pasture, 5-Water, 6-Urban)
