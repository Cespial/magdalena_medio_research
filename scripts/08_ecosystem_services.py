...
def compute_water_yield_proxy(lulc_map, region, year):
    ...
    recharge_values = {1: 0.35, 2: 0.30, 3: 0.15, 4: 0.18, 5: 0, 6: 0.05, 7: 0.10}
...
def compute_habitat_quality(lulc_map, region, urban_mask_t1=None):
    """
    ...
    Args:
        lulc_map: ee.Image with LULC classification (1-7)
        region: ee.Geometry
        urban_mask_t1: ee.Image (optional) Time-invariant urban mask from T1
            to eliminate the urban class artifact (206k→66k ha decline).
            If provided, this mask is used for urban threat distance
            computation in ALL periods, ensuring consistent habitat quality.
    """
    ...
    # FIX: Use ee.Image.distance() at 100m scale instead of
    # fastDistanceTransform at 30m to avoid GEE memory limit on T3
    agro_distance = agro_threat.selfMask().distance(
        kernel=ee.Kernel.euclidean(5000, 'meters'), skipMasked=False
    ).reproject(crs='EPSG:4326', scale=100).unmask(5000).rename('agro_dist')
    agro_decay = agro_distance.divide(5000).multiply(-1).exp()

    # Amenaza 2: Proximidad a urbano
    # FIX: Use TIME-INVARIANT urban mask from T1 to eliminate artifact
    if urban_mask_t1 is not None:
        urban_threat = urban_mask_t1
    else:
        urban_threat = lulc_map.eq(6)

    urban_distance = urban_threat.selfMask().distance(
        kernel=ee.Kernel.euclidean(10000, 'meters'), skipMasked=False
    ).reproject(crs='EPSG:4326', scale=100).unmask(10000).rename('urban_dist')
    urban_decay = urban_distance.divide(10000).multiply(-1).exp()

    # Amenaza 3: Proximidad a carreteras (usando suelo desnudo como proxy)
    road_threat = lulc_map.eq(7)
    road_distance = road_threat.selfMask().distance(
        kernel=ee.Kernel.euclidean(3000, 'meters'), skipMasked=False
    ).reproject(crs='EPSG:4326', scale=100).unmask(3000).rename('road_dist')
    road_decay = road_distance.divide(3000).multiply(-1).exp()
...
def main():
    ...
    # -----------------------------------------------------------------
    # FIX: Create time-invariant urban mask from T1 to eliminate artifact
    # The urban class showed an implausible decline from 206k to 66k ha
    # due to GHSL SMOD temporal inconsistency. Using T1 urban extent
    # for ALL periods ensures consistent habitat quality computation.
    # -----------------------------------------------------------------
    print("\n  Creating time-invariant urban mask from T1...")
    try:
        t1_info = PERIODS['pre_acuerdo']
        project_id = os.getenv('GEE_PROJECT_ID', 'ee-maestria-tesis')
        lulc_t1 = ee.Image(f'projects/{project_id}/assets/lulc_{t1_info["map_year"]}').clip(region)
        urban_mask_t1 = lulc_t1.eq(6)  # Urban class = 6
        print("    T1 urban mask created from classified LULC asset.")
    except Exception:
        # Fallback: use GHSL SMOD >= 30
        ghsl_smod = ee.Image('JRC/GHSL/P2023A/GHS_SMOD_V2-0').select('smod_code')
        urban_mask_t1 = ghsl_smod.gte(30)
        print("    Fallback: T1 urban mask from GHSL SMOD >= 30.")
    ...
    for period_key, period_info in PERIODS.items():
        ...
        print(f"    Habitat quality: threat proximity model (time-invariant urban mask)")
