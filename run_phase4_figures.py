...
LULC_ABBREV = {1: 'BDen', 2: 'BSec', 3: 'Past', 4: 'Cult', 5: 'Water', 6: 'Urban', 7: 'Bare'}
...
def fig04b_forest_trend(plt, data):
    """Forest area trend with Olofsson 95% CI shading."""
    clf = data['classification']
    ...
    total = [d + s for d, s in zip(dense, secondary)]
    total_ci = [np.sqrt(dc ** 2 + sc ** 2) for dc, sc in zip(dense_ci, secondary_ci)]
    ...
    # Dense and secondary individually
...
def fig05_transition_matrices(plt, data):
    """Clean heatmaps with English labels and adaptive text color."""
    chg = data['change']
...
def fig11_camarkov_scenarios(plt, data):
    """Simplified CA-Markov projections for 3 main classes."""
    cam = data['camarkov']
    ...
    # Focus on 3 main classes
    focus_classes = ['BDen', 'BSec', 'Past']
    class_labels = ['Dense\nforest', 'Secondary\nforest', 'Pastures']

    areas_2024 = cam['areas_2024']
    total_2024 = sum(areas_2024.get(c, 0) for c in ['BDen', 'BSec', 'Past', 'Agua', 'Urb'])
    ...
    for ax, yr, panel_label in [(ax1, '2030', '(a)'), (ax2, '2040', '(b)')]:
        ...
        ax.plot(x + w, baseline_pcts, 'ks--', markersize=4, linewidth=0.8,
               label='Current (2024)')
...
def table01_accuracy(data):
    ...
    with open(path, 'w', newline='') as f:
        ...
        for pk, label in zip(PERIOD_KEYS, PERIOD_LABELS):
            p = clf[pk]
            writer.writerow([
                label, p['year'], p.get('classification_method', 'Random Forest'),
...
def table06_camarkov_projections(data):
    ...
    with open(path, 'w', newline='') as f:
        writer = csv.writer(f)
        classes = ['BDen', 'BSec', 'Past', 'Cult', 'Agua', 'Urb', 'Suel']
        class_names = ['Dense Forest', 'Secondary Forest', 'Pastures',
                      'Crops', 'Water', 'Urban', 'Bare Soil']
